        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">Live overview of MediLedger operations</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white border border-slate-200/80 p-6 rounded-xl shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold uppercase text-slate-400 tracking-wider">Medicines catalog</span>
                  <h3 className="text-3xl font-black text-slate-800 mt-2">{medicines.length}</h3>
                </div>
                <div className="bg-indigo-50 text-indigo-500 p-4 rounded-lg"><Package size={24} /></div>
              </div>
              
              <div className="bg-white border border-slate-200/80 p-6 rounded-xl shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold uppercase text-slate-400 tracking-wider">Total Batches</span>
                  <h3 className="text-3xl font-black text-slate-800 mt-2">{batches.length}</h3>
                </div>
                <div className="bg-blue-50 text-blue-500 p-4 rounded-lg"><FolderOpen size={24} /></div>
              </div>

              <div className="bg-white border border-slate-200/80 p-6 rounded-xl shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold uppercase text-slate-400 tracking-wider">Registered Customers</span>
                  <h3 className="text-3xl font-black text-slate-800 mt-2">{customers.length}</h3>
                </div>
                <div className="bg-orange-50 text-orange-500 p-4 rounded-lg"><Users size={24} /></div>
              </div>

              <div className="bg-white border border-slate-200/80 p-6 rounded-xl shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold uppercase text-slate-400 tracking-wider">Sales recorded</span>
                  <h3 className="text-3xl font-black text-slate-800 mt-2">{sales.length}</h3>
                </div>
                <div className="bg-emerald-50 text-emerald-500 p-4 rounded-lg"><Receipt size={24} /></div>
              </div>
            </div>

            <div className="bg-blue-50 border border-blue-200/60 p-5 rounded-xl flex gap-3 text-blue-800">
              <LayoutDashboard size={20} className="shrink-0 mt-0.5" />
              <div>
                <h4 className="font-bold text-sm">Step 7 Real-time Billing Enabled</h4>
                <p className="text-xs text-blue-700/90 mt-1">
                  Navigate to the **Billing Counter** tab to generate invoices. The billing system implements the required first-expiry-first-out (FEFO) batching algorithm and auto-splits local GST (CGST + SGST) or interstate GST (IGST) in real-time.
                </p>
              </div>
            </div>
          </div>
